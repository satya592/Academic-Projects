...
public class MoleculeReader
{
	...
	public static Molecule readNative(BufferedReader in) throws IOException
	{
		...
		try
		{
			...
			for (int n=0;n<numBonds;n++)
			{
				line=in.readLine();
				String[] bits=line.split("[\\-\\=\\,]");
				if (bits.length<4) throw new MoleculeIOException(GENERIC_ERROR);
				int num=mol.addBond(Integer.parseInt(bits[0].trim()),Integer.parseInt(bits[1].trim()),
									Integer.parseInt(bits[2].trim()),Integer.parseInt(bits[3].trim()));
				extra.clear(); trans.clear();
				for (int i=4;i<bits.length;i++) if (bits[i].length()>0)
				{
	...
	public static Molecule readMDLMOL(BufferedReader in) throws IOException
	{
		...
		try
		{
			...
			if (!line.substring(34,39).equals("V2000")) throw new MoleculeIOException(GENERIC_ERROR);
			...
			while (true)
			{
				...
				else if (line.startsWith("M  ISO")) type=3;
				else if (line.startsWith("M  RGP")) type=4;
				...
				if (type>0)
				{
					...
					for (int n=0;n<len;n++)
					{
						...
						else if (type==3) mol.setAtomIsotope(apos,aval);
						else if (type==4) mol.setAtomElement(apos,"R"+aval);
					}
				}
			}
		}
